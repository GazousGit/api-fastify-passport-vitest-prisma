...
const microsoftRoutes: FastifyPluginAsync = async (app) => {
  if (!env.MICROSOFT_CLIENT_ID || !env.MICROSOFT_CLIENT_SECRET) {
    app.get('/microsoft', async (_request, reply) =>
    ...
    app.get('/microsoft/callback', async (_request, reply) =>
      reply
        .code(501)
        .send({ statusCode: 501, error: 'Not Implemented', message: 'Microsoft OAuth is not configured' }),
    )
    return
  }
  ...
  app.get(
    '/microsoft',
    { schema: { tags: ['auth'], summary: 'Initiate Microsoft OAuth flow' } },
    oauthHandler(authenticator.authenticate('microsoft', { scope: ['user.read'] })),
  )

  app.get(
    '/microsoft/callback',
    { schema: { tags: ['auth'], summary: 'Microsoft OAuth callback' } },
    oauthHandler(
