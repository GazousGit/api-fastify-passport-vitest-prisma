...
const facebookRoutes: FastifyPluginAsync = async (app) => {
  if (!env.FACEBOOK_CLIENT_ID || !env.FACEBOOK_CLIENT_SECRET) {
    app.get('/facebook', { schema: { tags: ['oauth-facebook'], summary: 'Initiate Facebook OAuth flow' } }, async (_request, reply) =>
      reply.code(501).send({ statusCode: 501, error: 'Not Implemented', message: 'Facebook OAuth is not configured' }),
    )
    app.get('/facebook/callback', { schema: { tags: ['oauth-facebook'], summary: 'Facebook OAuth callback' } }, async (_request, reply) =>
      reply.code(501).send({ statusCode: 501, error: 'Not Implemented', message: 'Facebook OAuth is not configured' }),
    )
    return
  }
  ...
  app.get(
    '/facebook',
    { schema: { tags: ['oauth-facebook'], summary: 'Initiate Facebook OAuth flow' } },
    oauthHandler(authenticator.authenticate('facebook', { scope: ['email'] })),
  )

  app.get(
    '/facebook/callback',
    { schema: { tags: ['oauth-facebook'], summary: 'Facebook OAuth callback' } },
